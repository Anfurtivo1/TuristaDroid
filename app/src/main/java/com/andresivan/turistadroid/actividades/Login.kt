...
class Login : AppCompatActivity() {
    ...
    private fun iniciarSesion() {
        ...
        if (isFormularioRelleno()) {
            ...
            val clienteREST = MisSitiosAPI.service
            val call: Call<UsuarioDTO> = clienteREST.getUsuarioByID(usuario.id)
            call.enqueue((object : retrofit2.Callback<UsuarioDTO> {
                override fun onResponse(
                    call: Call<UsuarioDTO>,
                    response: Response<UsuarioDTO>
                ) {
                    if (response.isSuccessful) {
                        Log.i("TuristaREST", "getUsuarioByID RESPUESTA OK")
                        val usuario = UsuarioMapeado.fromDTO(response.body() as UsuarioDTO)
